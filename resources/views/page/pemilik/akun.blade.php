...
<body class="bg-gray-100 font-sans">
    ...
    <div class="flex-1 container mx-auto p-4">
        ...
        <button @click="showAddModal = true"
            class="btn-add px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 mb-4">
        ...
        <div x-show="showAddModal" x-cloak class="fixed inset-0 flex items-center justify-center z-50">
            <div class="modal-overlay fixed inset-0" @click="showAddModal=false"></div>
            <div class="bg-white rounded-lg w-full max-w-md shadow-lg z-50 p-6">
                ...
                <form @submit.prevent="submitAddUser" class="space-y-4">
                    ...
                    <div>
                        <label>Email</label>
                        <input type="email" x-model="formData.email" class="w-full border rounded px-3 py-2" required>
                    </div>
                    <div>
                        <label>No WA</label>
        ...
        function app() {
            return {
                ...
                async submitVerificationCode() {
                    ...
                    try {
                        const res = await fetch("{{ route('pemilik.akun.verify') }}", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        ...
                        if (this.pendingAccessUserId) {
                            ...
                            const btn = document.querySelector(`button[data-user-id='${this.pendingAccessUserId}']`);
                            if (btn) {
                ...
                async handleAccessClick(userId, name, currentAccess, email) {
                    ...
                    try {
                        const res = await fetch(`/akun-owner/${userId}/resend-code`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                ...
                async toggleAccess(userId) {
                    ...
                    try {
                        const res = await fetch(`/akun-owner/${userId}/access`, {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
